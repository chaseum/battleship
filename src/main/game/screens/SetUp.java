...
import src.main.game.logic.Ship;
...
public class SetUp extends JPanel implements ActionListener, KeyListener {
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == timer) {
			...
			if (waitingForName) {
				...
				nameField.setBackground(Helper.interpolate(Helper.buttonBg, Helper.highlighted,
	...
	public void drawTarget(Graphics g) {
		if (targetX != -1) {
			g.drawRect(targetX * Helper.cellWidth + 472, 142 + targetY * Helper.cellWidth, Helper.cellWidth,
					Helper.cellWidth);
		}
	}
	...
	public boolean isValidPlacement() {
		if (placing == null || targetX + (horizontal ? placing.getLength() : 0) > 10
				|| targetY + (!horizontal ? placing.getLength() : 0) > 10) {
			return false;
		}
	...
	public void paintComponent(Graphics g) {
		...
		if (shipsToDraw != null) {
			for (Ship ship : shipsToDraw) {
				g.drawRect(Helper.fixWidth2(1160), Helper.fixHeight2(142) + i * Helper.fixHeight2(75),
						(int) (Helper.fixWidth2(275) * ship.getLength() / 5.0), Helper.fixHeight2(50));
				i++;
			}
		}
		if (targetX != -1) {
			if (shipsPlaced != null) {
				for (Ship ship : shipsPlaced) {
					...
					g.setColor(Color.BLUE);
					g.fillRect(ship.getX() * Helper.cellWidth + Helper.fixWidth2(473),
							Helper.fixHeight2(143) + Helper.cellWidth * ship.getY(),
			...
			if (placing != null) {
				...
				g.fillRect(placing.getX() * Helper.cellWidth + Helper.fixWidth2(472),
						Helper.fixHeight2(142) + Helper.cellWidth * placing.getY(),
						Helper.cellWidth * (horizontal ? placing.getLength() : 1),
						Helper.cellWidth * (horizontal ? 1 : placing.getLength()));
