...
public class Title extends JPanel implements ActionListener, KeyListener {
	...
	public Title(JFrame frame) {
		...
		solo.setPreferredSize(new Dimension(Helper.fixWidth(750), Helper.fixHeight(150)));
		...
		solo.addActionListener(e -> {
			Helper.showCard(frame, "SETUP");
			frame.getContentPane().add(new SetUp(frame), "SETUP");
		});
		...
		c.gridy = 2;
		c.gridwidth = 2;
		c.insets = new Insets(Helper.fixHeight(40), 0, 0, 0);
		this.add(solo, c);
		exit.setBounds(frame.getWidth() - 200, frame.getHeight() - 120, 160, 50);

		if (getParent() == null) {
			frame.getContentPane().add(this, "TITLE");
		}
	...
	public void keyPressed(KeyEvent e) {
		...
		if (key.equals("Enter")) {
			...
			if (joinSelected) {
				...
				((CardLayout) frame.getContentPane().getLayout()).show(frame.getContentPane(), "HOST");
			}
			frame.getContentPane().revalidate();
			frame.getContentPane().repaint();
		}
